...
/// Skeleton placeholder shown while the orders list is loading.
/// Mirrors the column structure of [OrderTable] so the layout
/// feels stable as real data replaces it.
class OrderTableSkeleton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(builder: (context, constraints) {
      final isDesktop = constraints.maxWidth >= AppConstants.desktopBreakpoint;
      final columns = isDesktop ? OrderTable._columns : OrderTable._columnsMobile;
        ...
        children: [
          ...
          Expanded(
            child: ListView.builder(
              ...
              itemCount: 10,
              itemBuilder: (_, i) => _SkeletonRow(isEven: i.isEven, columns: columns),
            ),
          ),
        ],
      );
...
class _SkeletonRow extends StatelessWidget {
  const _SkeletonRow({required this.isEven, required this.columns});
              ...
              child: Column(
                ...
                children: [
                  Container(
                    ...
                    decoration: BoxDecoration(
                      color: AppColors.textSecondary.withAlpha(35),
                      borderRadius: BorderRadius.circular(4),
                    ),
                  ),
                  if (col.flex != null) ...[
...
class _SkeletonCellWrapper extends StatelessWidget {
  const _SkeletonCellWrapper({required this.column, required this.child});
