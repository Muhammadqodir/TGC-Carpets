...
class ProductAnalyticsService
{
    ...
    /**
     * Return filtered & limited top products for the dedicated top-products endpoint.
     * Supports filtering by type, quality, size, color, edge and a configurable limit.
     */
    public function getTopProducts(
        ...
        $ttl      = $this->resolveTtl($to);
        $cacheKey = "analytics:top-products:{$from}:{$to}:{$limit}:{$typeId}:{$qualityId}:{$sizeId}:{$colorId}:{$edgeId}";

        return Cache::remember($cacheKey, $ttl, function () use (
            $from, $to, $limit, $typeId, $qualityId, $sizeId, $colorId, $edgeId
        ): array {
            return $this->queryTopProductsFiltered(
                $from, $to, $limit, $typeId, $qualityId, $sizeId, $colorId, $edgeId
            );
        });
    }
    ...
    private function queryTopProductsFiltered(
        string $from,
        string $to,
        int    $limit    = 10,
        ?int   $typeId   = null,
        ...
        $total = $this->totalItemsFiltered($from, $to, $typeId, $qualityId, $sizeId, $colorId, $edgeId);
        ...
        if ($sizeId !== null) {
            $query->where('product_variants.product_size_id', $sizeId);
        }
        if ($colorId !== null) {
            $query->where('product_colors.color_id', $colorId);
        }
        if ($edgeId !== null) {
            $query->where('product_variants.product_edge_id', $edgeId);
        }
        if ($typeId !== null) {
            $query->where('products.product_type_id', $typeId);
        }
        if ($qualityId !== null) {
            $query->where('products.product_quality_id', $qualityId);
        }
    ...
    private function totalItemsFiltered(
        string $from,
        string $to,
        ?int   $typeId   = null,
        ?int   $qualityId = null,
        ...
        return (int) $query->selectRaw('COALESCE(SUM(order_items.quantity), 0) as total')
            ->value('total');
    }
