...
- **phase-2/** — the data structure upgrade. 8 steps, ~3 weeks. The main event. All 8 are implemented — see [phase-2/DEPLOY.md](phase-2/DEPLOY.md) for the staged rollout (foundation → backfills → alerting → client release → analytics sign-off; the stock-read switch is deliberately deferred past this deploy), [phase-2/reconcile-before-deploy.sql](phase-2/reconcile-before-deploy.sql) for the read-only queries to run first, and [phase-2/COMPLETION-REPORT.md](phase-2/COMPLETION-REPORT.md) for full status and handoff context.
- **phase-3/** — make it an advanced ERP. 8 steps, ongoing. All 8 are implemented — see [phase-3/DEPLOY.md](phase-3/DEPLOY.md) for the staged rollout (foundation → unit backfill → drift-report schedule → audit-log lockdown; the currency selector, VAT, reservation hard-block, and printing the real unit serial are all deliberately deferred past this deploy), [phase-3/reconcile-before-deploy.sql](phase-3/reconcile-before-deploy.sql) for the read-only queries to run first, and [phase-3/COMPLETION-REPORT.md](phase-3/COMPLETION-REPORT.md) for full status, the step-03 Path A/B decision made without the user present, and handoff context.
...
Phase-0 (12 steps), phase-1 (8 of 9 steps — 09 was dropped), phase-2 (8 of 8 steps), and phase-3 (8 of 8 steps) are code-complete but **not yet deployed to production**; see each phase's own `COMPLETION-REPORT.md` for exactly what that means and what's still unverified. Phase-2 additionally leaves two rollout decisions deliberately deferred past the initial deploy (the `ANALYTICS_SOURCE` flip and the stock-balance read switch) — see `phase-2/DEPLOY.md`. Phase-3 leaves several more deferred by design (currency/VAT client UI, the reservation hard-block, printing the real unit serial, and `production_units` becoming authoritative) — see `phase-3/DEPLOY.md` and its completion report, which also documents a step-03 decision (Path A vs Path B) made without the user present that should be revisited once there is database access. If you complete a step, note it in that step's file rather than deleting the file — the reasoning is worth keeping.
